...
class UpdateDatabaseSyncGroupCommand(UpdateMixin, BaseCommand):
    ...
    def run(self):
        self.validate()
        DatabaseSyncGroupDAO.update(self._model, self._properties, commit=False)
        if hasattr(self._properties, "level"):
            DatabaseSyncGroupDAO.update_children_level(
    ...
    def validate(self):
        ...
        # 移动子目录事件
        ids = DatabaseSyncGroupDAO.find_children_ids_by_pid(self._model_id)
        # 判断目标分组是否是子目录
        if self._properties["pid"] in ids:
            raise HTTPError(Messages.MOVE_TO_CHILDREN_ERR, 400)

        # 校验目标分组是否存在
        parent_group = DatabaseSyncGroupDAO.find_by_id(self._properties["pid"])
