...
from superset.v2.datasources.dao import DataBaseDAO
...
class DatabaseDataSourceListCommand(BaseCommand):
    ...
    def run(self, **kwargs: Any) -> List[dict]:
        self.validate()
        res = DataBaseDAO.find_databases()
        ...
        data_auth = DataBaseDAO.find_auth_source_perm_by_user(
            AuthSourceType.DATABASE_SYNC, self._actor.id, VIEW)
        return [
            {
                "id": item[0].database_id,
                "database_name": item[0].name,
