...
class MoveDatasetV2Command(CreateMixin, BaseCommand):
    ...
    def run(self) -> Model:
        ...
        DatasetV2DAO.update(
            self._model,
            {
                "table_group_id": self._properties["target_id"],
                "custom_name": self._properties["new_title"],
            }
        )
        return self._model
    ...
    def validate(self) -> None:
        ...
        # 重命名事件
        if self._properties["target_id"] == self._model.table_group_id:
            return
        ...
        # 查询分组是否存在
        group = DatasetGroupDAO.find_by_id(self._properties["target_id"])
        if group is None:
            raise HTTPError(Messages.GROUP_NOT_EXIST, 400)
