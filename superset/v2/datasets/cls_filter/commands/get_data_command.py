...
class CLSFilterDataCommand(BaseCommand):
    ...
    def run(self, **kwargs: Any) -> List[Any]:
        self.validate()
        return self._model.cls_filter_data
...
class DatasetCLSFilterDataCommand(BaseCommand):
    def __init__(self, user: User, model_id: int):
        self._actor = user
        self._model_id = model_id
        self._model = None
    ...
    def run(self, **kwargs: Any) -> List[Any]:
        self.validate()
        return [item.cls_filter_data for item in self._model.columns]
