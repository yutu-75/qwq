...
class CreateDatasetGroupCommand(CreateMixin, BaseCommand):
    ...
    def run(self):
        self.validate()
        group = DatasetGroupDAO.create(self._properties)
        # 写入权限
    ...
    def validate(self) -> None:
        ...
        if pid == 0:
            menu = SysMenuDAO.find_by_name(AuthType.DATASET)
            menu.can_access(MANAGE)
            level = 0

        else:
            group = DatasetGroupDAO.find_by_id(pid)
            if group is None:
