...
class DeleteDataSourceGroupCommand(BaseCommand):
    def __init__(self, user: User, model_id: int):
        self._actor = user
        self._model_id = model_id
        self._model: Optional[DataSourceGroup] = None

    def run(self) -> Model:
        self.validate()
        group_ids = DataSourceGroupDAO.find_del_group_ids(
            DataSource, self._model_id, [self._model_id]
        )
        datasource = DataSourceDAO.find_by_group_ids(group_ids)
        if datasource:
            raise HTTPError(Messages.DEL_GROUP_ERROR, 400)
        DataSourceGroupDAO.delete_by_ids(group_ids)
        DataSourceGroupDAO.delete_auth_by_source_ids(
    ...
    def validate(self) -> None:
        self._model = DataSourceGroupDAO.find_by_id(self._model_id)
        if not self._model:
        ...
        # DataSourceGroupDAO.check_group_is_empty(self._model_id)

    def archiving(self):
        doc_json = self._model.to_json()
        DatabaseDAO.archiving_docs("datasource_group", doc_json)
