...
from superset.models.datasource import DataSourceGroup
from superset.v2.datasources.group.dao import DataSourceGroupDAO

logger = logging.getLogger(__name__)


class UpdateDataSourceGroupCommand(BaseCommand):
    ...
    def run(self) -> Model:
        self.validate()
        model = DataSourceGroupDAO.update(self._model, self._properties, commit=False)
        if hasattr(self._properties, "level"):
            DataSourceGroupDAO.update_children_level(
    ...
    def validate(self):
        ...
        # 移动子目录事件
        ids = DataSourceGroupDAO.find_children_ids_by_pid(self._model_id)
        # 判断目标分组是否是子目录
        if self._properties["pid"] in ids:
            raise HTTPError(Messages.MOVE_TO_CHILDREN_ERR, 400)

        # 检查当前分组是否存在
        parent_group = DataSourceGroupDAO.find_by_id(self._properties["pid"])
