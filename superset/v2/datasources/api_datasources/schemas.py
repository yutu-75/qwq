...
from marshmallow.validate import Length, Range, Equal
...
class TokenAPIListResponseSchema(Schema):
    api_table_id = fields.Integer()
    name = fields.String()
...
class ApiDataSourceTestSchema(Schema):
    ...
    method = fields.Enum(MethodType, required=True)
    headers = fields.Dict(required=False)
    params = fields.Dict(required=False)
    data = fields.Dict(required=False)
    json = fields.Dict(required=False)
    auth = fields.Dict(required=False)
    token_api = fields.Dict(required=False)
...
class ApiTablePostSchema(Schema):
    ...
    data_path = fields.Nested(DataPathSchema, required=True)
    extra = fields.String()
    status = fields.Integer(required=False, validate=Range(0, 1))


class ApiDataSourcePostSchema(Schema):
    name = fields.String(required=True, validate=Length(1, 50))
    desc = fields.String(required=False, validate=Length(0, 50))
    group_id = fields.Integer(required=True, validate=Range(min=0))
    d_type = fields.String(required=True)
    tables = fields.List(fields.Nested(ApiTablePostSchema))


class ApiDataSourcePutSchema(Schema):
    name = fields.String(required=True, validate=Length(1, 50))
    desc = fields.String(required=False, allow_none=True, validate=Length(0, 50))
    tables = fields.List(fields.Nested(ApiTablePostSchema))
    d_type = fields.String(required=True)
