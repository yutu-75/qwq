...
from superset.models.database_sync import DatabaseSyncTask
...
class DataSourceDAO(BaseDAO):
    ...
    @staticmethod
    def find_by_uuid(uuid: str) -> DataSource:
        datasource = db.session.query(DataSource).filter_by(
            uuid=uuid
        ).one_or_none()
        return datasource

    @staticmethod
    def find_by_database_id(database_id: int) -> DataSource:
        datasource = db.session.query(DataSource).filter_by(
    ...
    def validate_uniqueness(cls, name: str, group_id: int) -> bool:
        datasource_query = db.session.query(DataSource).filter(
    ...
    def find_id_by_name(name: str) -> int:
        datasource = db.session.query(DataSource.id).filter(
            DataSource.name == name
        ).one_or_none()
        return 0 if datasource is None else datasource[0]

    @staticmethod
    def find_by_group_ids(group_ids: list) -> int:
        datasource = db.session.query(DataSource.id).filter(
            DataSource.group_id.in_(group_ids)
        ).all()
        return datasource

    @classmethod
    def find_by_type(cls, d_type: str) -> List[DataSource]:
    ...
    @classmethod
    def find_databases(cls) -> List[DataSource]:
