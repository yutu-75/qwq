...
class ExportDatabasesCommand(ExportModelsCommand):
    ...
    def _export(
        self, model: Database, export_related: bool = True
    ) -> Iterator[Tuple[str, str]]:
        database_slug = model.database_name
        file_name = f"databases/{database_slug}_{model.id}.yaml"
        ...
        datasource = DataSourceDAO.find_by_id(self._datasource_id)
        payload["datasource"] = {
            ...
            "group_path": DataSourceGroupDAO.export_group(datasource.group_id)
        }
        ...
        event_logger.log_with_context(
            action="导出数据源",
            log_to_statsd=False,
            title=payload["datasource"].get('name', '')
        )
        yield file_name, file_content

        if export_related:
            for dataset in model.tables:
                dataset_slug = f'{dataset.table_name}_{dataset.id}'
                ...
                payload["group_path"] = DatasetGroupDAO.export_group(
                    dataset.table_group_id)
