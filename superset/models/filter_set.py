...
class FilterSet(Model, AuditMixinNullable):
    ...
    json_metadata = Column(Text, nullable=False, comment='json 元数据')
    dashboard_id = Column(Integer, ForeignKey("dashboards.id"))
    dashboard = relationship("Dashboard", back_populates="_filter_sets")
    owner_id = Column(Integer, nullable=False, comment='归属 id')
    owner_type = Column(String(255), nullable=False, comment='归属类型')
