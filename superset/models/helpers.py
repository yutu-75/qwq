...
from superset.constants import EMPTY_STRING, NULL_STRING, AuthTargetType
...
def _user_link(user: User) -> Union[Markup, str]:
    if not user:
        return ""
    url = "{}/superset/profile/{}/".format(config['STATIC_ASSETS_PREFIX'], user.username)
    return Markup('<a href="{}">{}</a>'.format(url, escape(user) or ""))


class PermissionMixin:
    @staticmethod
    def _add_user_db_permission(
        database_id: int,
        ...
        DatabaseAuthDAO.insert_or_update({
            "database_id": database_id,
            "auth_source_type": auth_source_type,
            "auth_target": g.user.id,
    ...
    def _can_access(
        ...
        from superset.sys_manager.permission.dao import SysAuthDAO
        if not SysAuthDAO.validate_auth_source(
            auth_source=auth_source,
            auth_source_type=auth_source_type,
            privilege_value=privilege_value,
        ) and not getattr(g.user, 'is_guest_user', None):
            # getattr(g.user, 'is_guest_user', None) 跳过非注册用户的权限认证
            raise HTTPError(Messages.FORBIDDEN, 403)


class AuditMixinNullable(AuditMixin, PermissionMixin):
    """Altering the AuditMixin to use nullable fields

    Allows creating objects programmatically outside of CRUD
    """

    created_on = sa.Column(sa.DateTime, default=datetime.now, nullable=True, comment="创建时间")
    changed_on = sa.Column(
        sa.DateTime, default=datetime.now, onupdate=datetime.now, nullable=True, comment="更新时间"
    )
...
class ExtraJSONMixin:
    ...
    extra_json = sa.Column(LONGTEXT, default="{}", comment="额外的信息")
