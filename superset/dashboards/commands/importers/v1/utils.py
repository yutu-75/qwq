...
def import_dashboard(
    ...
    if dash_group_id == 0:
        group_path = config.pop("group_path", False)
        group = DashboardGroupDAO.import_group(group_path)
        dash_group_id = group.id

    config["dashboard_group_id"] = dash_group_id
    ...
    dashboard.add_user_permission(GRANT)
    return dashboard
