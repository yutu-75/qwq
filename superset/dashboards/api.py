...
def with_dashboard(
    ...
    def wraps(self: BaseSupersetModelRestApi, id_or_slug: str) -> Response:
        try:
            ...
            return f(self, dash)
        except (DashboardAccessDeniedError, HTTPError):
...
class DashboardRestApi(BaseSupersetModelRestApi):
    ...
    def get(
        ...
        result["perm"] = DashboardDAO.find_perm(dash.id, AuthType.DASHBOARD)
    ...
    def get_charts(self, id_or_slug: str) -> Response:
        ...
        try:
            ...
            if isinstance(parse_qs(urlparse(request.referrer).query).get('view'), list):
                g.user.is_admin = True

            # 判断是否是访客用户
            if g.user.username == "guest_user":
                g.user.is_admin = True

            if (
                g.user.is_admin or
                session.get("_user_source") == current_app.config["CAPS"]
            ):
                result = [{
                    "perm": DashboardDAO.find_perm(chart.id, AuthType.CHART),
                    **self.chart_entity_response_schema.dump(chart)
                } for chart in charts]
            else:
                # 数据权限
                data_auth = DashboardDAO.find_auth_source_perm_by_user(
                    AuthSourceType.CHART, g.user.id, 1)
                result = [{
                    "perm": DashboardDAO.find_perm(chart.id, AuthType.CHART),
                    **self.chart_entity_response_schema.dump(chart)
                } for chart in charts if chart.id in data_auth]
    ...
    @statsd_metrics
    @event_logger.log_this_with_context(
        action=LogsMessages.LM_EDIT_DASHBOARD,
        log_to_statsd=False,
    )
    @requires_json
    def put(self, pk: int) -> Response:
        ...
        except (DashboardForbiddenError, HTTPError):
    ...
    def import_(self) -> Response:
        ...
        dash_group_id = int(request.form.get("dash_group_id", 0))
