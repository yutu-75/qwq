...
# Created:          2023/6/15
# Rows:             386
# Description:      重写 _get_notification_content()
#                   添加 pdf context
#                   原有函数保留 (row-331)
#                   _get_notification_content 重命名为 _get_notification_content_old
# ------------------------------------------------------------------
...
from superset.v2.utils.data_save_db import get_save_default_db, api_data_to_df, \
    df_to_sql
...
class BaseReportState:
    ...
    def _get_screenshots(self) -> List[bytes]:
        ...
        creator = self._report_schedule.created_by
        if self._report_schedule.chart:
        ...
        try:
            image = screenshot.get_screenshot(user=user, creator=creator)
    ...
    # def _get_notification_content_old(self) -> NotificationContent:
    #     """
    ...
    def _get_notification_content(self) -> NotificationContent:
        ...
        if (
            ...
            # 非邮件不需要截图
            if self._report_schedule.recipients[0].type != 'Email':
                pass
            # png
            if self._report_schedule.report_format == ReportDataFormat.VISUALIZATION:
                screenshot_data = self._get_screenshots()
    ...
    def _get_pdf(self) -> bytes:
        ...
        try:
            pdf_data = screenshot.get_pdf(user=user)
        except SoftTimeLimitExceeded as ex:
            logger.warning("A timeout occurred while taking a screenshot.")
            raise ReportScheduleScreenshotTimeout() from ex
        except Exception as ex:
            raise ReportScheduleScreenshotFailedError(
                f"Failed taking a pdf {str(ex)}"
            ) from ex
        if not pdf_data:
            raise ReportScheduleScreenshotFailedError()
        return pdf_data
...
class APIDatasetScheduleStateMachine:  # pylint: disable=too-few-public-methods
    """
    Simple state machine for Alerts/Reports states
    """
    ...
    def __init__(
        ...
        session: Session,
        task_uuid: UUID,
        dataset_schedule: Model,
        scheduled_dttm: datetime,
        user: 'User',
    ):
        self._session = session
        self._execution_id = task_uuid
        self._model = dataset_schedule
        self._scheduled_dttm = scheduled_dttm
        self._user = user

    def run(self) -> None:
        last_exec_time = self._scheduled_dttm
        logger.debug(last_exec_time)
        start_time = int(last_exec_time.timestamp() * 1000)
        log = TablesTaskLog(
            table_task_id=self._model.id,
            task_id=self._execution_id,
            start_time=start_time,
        )
        logger.debug("start_time - current_time: %s", (self._model.start_time - start_time,))
        if self._model.start_time > start_time:
            return

        logger.debug("end_time - current_time: %s", (self._model.end_time - start_time,))
        if 0 < self._model.end_time < start_time:  # 结束时间已到自动转为关闭状态
            logger.info("self._model.end_time: %s" % self._model.end_time)
            logger.info("start_time: %s" % start_time)
            log.info = 'Expired'
            log.end_time = start_time
            log.status = 0
            self._model.last_exec_status = log.info
            self._model.status = log.status

        else:
            try:
                self.update_api_dataset()
                self._model.last_exec_status = "Completed"
                log.info = "Completed"
                log.status = 1
            except Exception as ex:
                logger.error(ex)
                log.info = str(ex)
                log.status = 0
                self._model.last_exec_status = "Failed"
                self._model.status = log.status
                # raise ReportScheduleUnexpectedError(str(ex)) from ex

            log.end_time = int(datetime.now().timestamp() * 1000)

        self._model.last_exec_time = last_exec_time
        self._session.add(log)
        self._session.merge(self._model)
        self._session.commit()

    def update_api_dataset(self):
        database = get_save_default_db(self._user)
        api_table = self._model.api_table
        df = api_data_to_df(
        ...
        df_to_sql(
            ...
            app.config["UPLOAD_SCHEMA"],
            if_exists=self._model.update_type
        )
...
class AsyncExecuteAPIDatasetsScheduleCommand(BaseCommand):
    def __init__(self, task_id: str, model_id: int, scheduled_dttm: datetime):
        self._model_id = model_id
        self._model: Optional[ReportSchedule] = None
        self._scheduled_dttm = scheduled_dttm
    ...
    def run(self) -> None:
        with session_scope(nullpool=True) as session:
            try:
                ...
                user = security_manager.find_user_by_id(self._model.changed_by_fk)
                with override_user(user):
                    logger.info(
                        "Running report schedule %s as user %s",
                        self._execution_id,
                    ...
                    APIDatasetScheduleStateMachine(
                        session,
                        self._execution_id,
                        self._model,
                        self._scheduled_dttm,
    ...
    def validate(  # pylint: disable=arguments-differ
        ...
        self._model = (
            session.query(TablesTask).filter_by(id=self._model_id).one_or_none()
        )
        if not self._model:
            raise ReportScheduleNotFoundError()
