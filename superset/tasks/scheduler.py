...
from superset.reports.commands.execute import AsyncExecuteReportScheduleCommand, \
    AsyncExecuteAPIDatasetsScheduleCommand
...
@celery_app.task(name="dataset.scheduler")
def api_dataset_scheduler() -> None:
    """
    Celery beat main scheduler for api dataset
    """
    # if not is_feature_enabled("ALERT_REPORTS"):
    #     return
    with session_scope(nullpool=True) as session:
        active_schedules = TableTaskDAO.find_active(session)
        for active_schedule in active_schedules:
            active_schedule.timezone = 'Australia/Perth'
            active_schedule.working_timeout = 50
            for schedule in cron_schedule_window(
                active_schedule.cron, active_schedule.timezone
            ):
                logger.info(
                    "Scheduling alert %s eta: %s", active_schedule.name, schedule
                )
                ...
                api_dataset_execute.apply_async(
                    (
                        active_schedule.id,
                        str(schedule),
                    ),
...
@celery_app.task(name="dataset.execute", bind=True)
def api_dataset_execute(self: Celery.task, report_schedule_id: int, scheduled_dttm: str) -> None:
    task_id = None
    try:
        task_id = self.request.id
        scheduled_dttm_ = parser.parse(scheduled_dttm)
        logger.info(
            "Executing api dataset, task id: %s, scheduled_dttm: %s",
            task_id,
            scheduled_dttm,
        )
        AsyncExecuteAPIDatasetsScheduleCommand(
