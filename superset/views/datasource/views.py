...
class Datasource(BaseSupersetView):
    ...
    def save(self) -> FlaskResponse:
        ...
        # python3 version compatible
        datasource_dict = json.loads(data, strict=False)
        ...
        if not datasource_dict.get("custom_name"):
            datasource_dict["custom_name"] = datasource_dict["table_name"]

        datasource_dict["table_group_id"] = orm_datasource.table_group_id
        datasource_dict["type_classify"] = orm_datasource.type_classify
    ...
    def external_metadata_by_name(self, **kwargs: Any) -> FlaskResponse:
        ...
        try:
            if datasource is not None:
                ...
                verbose_name = {
                    item.column_name: item.verbose_name or ""
                    for item in datasource.columns
                }
            else:
        ...
        columns_comment = dict()
        if datasource.type_classify == DatasetType.DATABASE:
            columns = database.get_columns(
                params["table_name"],
                params["schema_name"]
            )
            columns_comment = {
                item["name"]: item.get("comment") or "" for item in columns
            }

        for item in external_metadata:
            item['verbose_name'] = verbose_name.get(item['name'], "")
            item['comment'] = columns_comment.get(item['name'], "")
        return self.json_response(external_metadata)
