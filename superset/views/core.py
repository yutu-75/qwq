...
from superset.views.dashboard.views import base_64
import logging
import re
...
from superset.constants import QUERY_EARLY_CANCEL_KEY, MANAGE, MenuName, GRANT
...
from superset.exceptions import (
    ...
    SupersetTimeoutException, HTTPError,
)
...
HTML_TEMPLATE = "superset/spa.html"
basic_html = "superset/basic.html"
...
class Superset(BaseSupersetView):  # pylint: disable=too-many-public-methods
    ...
    def save_or_overwrite_slice(
        ...
        """
        if request.args.get("group_id", False):
            slc.slice_group_id = request.args.get("group_id", 0)
    ...
    def copy_dash(  # pylint: disable=no-self-use
        ...
        dash.dashboard_group_id = data.get("dashboard_group_id",
                                           original_dash.dashboard_group_id)

        if not DashboardV2DAO.validate_uniqueness(
            dash.dashboard_title,
            dash.dashboard_group_id,
        ):
            return json_error_response("看板名称重复", 400)

        old_to_new_slice_ids: Dict[int, int] = {}
        if data["duplicate_slices"]:
            # Duplicating slices as well, mapping old ids to new ones
            slice_group_id = data.get("slice_group_id", None)
            for slc in original_dash.slices:
                new_slice = slc.clone()
                new_slice.slice_name = new_slice.slice_name + "复制"
                if slice_group_id:
                    new_slice.slice_group_id = slice_group_id
        ...
        dash.mobile_json_metadata = original_dash.mobile_json_metadata
        DashboardDAO.set_dash_metadata(dash, data, old_to_new_slice_ids)
        session.add(dash)
        dash.add_user_permission(GRANT)
        session.commit()
    ...
    def fave_dashboards(self, user_id: int) -> FlaskResponse:
        ...
        for o in qry.all():
            ...
            if o.Dashboard.created_by:
                ...
                dash["creator_url"] = "{}/superset/profile/{}/".format(config['STATIC_ASSETS_PREFIX'], user.username)
            payload.append(dash)
        return json_success(json.dumps(payload, default=utils.json_int_dttm_ser))
    ...
    def warm_up_cache(  # pylint: disable=too-many-locals,no-self-use
        ...
        if isinstance(result, list):
            return json_success(str(result))
    ...
    @expose("/embedded/dashboard/<dashboard_id_or_slug>/")
    ...
    def dashboard(
        ...
        try:
            ...
            raise HTTPError('您当前无此看板权限，请联系分享人或系统管理员', 403)

        third_t = request.args.get('control')
        if third_t:
            third_tag = base_64(third_t)
            dashboard.third_tags = third_tag
            db.session.commit()
        ...
        # if not ("superset/dashboard/" in request.path):
        #     dashboard.can_access(MANAGE)
    ...
    def dashboard_permalink(  # pylint: disable=no-self-use
        ...
        # 增加前缀/superset
        url = f"{conf.get('STATIC_ASSETS_PREFIX')}/superset/dashboard/{dashboard_id}?permalink_key={key}"
        if request.args.get('timestamp') or view:
            url = f"{url}&view={view}"
        url_params = state.get("urlParams")
        if url_params:
    ...
    def search_queries(self) -> FlaskResponse:  # pylint: disable=no-self-use
        ...
        if security_manager.can_access_all_queries():
            ...
            try:
                ...
                return Response(status=400, mimetype=json_mimetype)
            if search_user_id != get_user_id():
                return Response(status=403, mimetype=json_mimetype)
        else:
    ...
    def profile(self, username: str) -> FlaskResponse:
        ...
        user = (
            db.session.query(UserV2).filter_by(username=username).one_or_none()
        )
    ...
    def sqllab(self) -> FlaskResponse:
        """SQL Editor"""
        menu = SysMenuDAO.find_by_name(MenuName.SQL_LAB)
        menu.can_access()
        payload = {
        ...
        return self.render_template(
            basic_html, entry="sqllab", bootstrap_data=bootstrap_data
        )
    ...
    @expose("/share/dashboard/<int:dashboard_id>/", methods=["GET"])
    def dashboard_share(self, dashboard_id: int) -> FlaskResponse:
        dashboard = DashboardDAO.get_title(dashboard_id)
    ...
    @expose("/board/share", methods=["GET"])
    def dashboard_share_login(self) -> FlaskResponse:
        bootstrap_data = {
            "common": common_bootstrap_payload(g.user),
        }
        return self.render_template(
