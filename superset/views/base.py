...
from ..constants import SystemConfigType
from ..sys_manager.sys_config.dao import SysConfigDAO
...
def json_success(json_msg: str, status: int = 200) -> FlaskResponse:
    return Response(json_msg, status=status, mimetype=application_json)
...
def menu_data(user: User) -> Dict[str, Any]:
    ...
    # 根据配置文件给菜单栏排序
    if conf.get("MENU_BAR_SORTING"):
        menu = sorted(menu, key=lambda x: conf.get("MENU_BAR_SORTING").index(x['name']))
    ...
    return {
        ...
        "navbar_right": {
            ...
            else f"{conf['STATIC_ASSETS_PREFIX']}/superset/profile/{user.username}",
            "locale": session.get("locale", "en"),
        },
    }


@cache_manager.cache.memoize(timeout=4)
def cached_common_bootstrap_data(user: User) -> Dict[str, Any]:
    ...
    # 从数据库更新配置参数
    appearance = SysConfigDAO.find_by_type(SystemConfigType.APPEARANCE)
    if appearance:
        conf.update({item.param_key: item.param_value for item in appearance})
...
def show_http_exception(ex: HTTPException) -> FlaskResponse:
    ...
    if (
        text_html in request.accept_mimetypes
...
def show_command_errors(ex: CommandException) -> FlaskResponse:
    ...
    if text_html in request.accept_mimetypes and not config["DEBUG"]:
...
@superset_app.errorhandler(HTTPError)
def handle_exception(e):
    logger.warning(e.message)
    return Response(
        json.dumps({"meta": {"code": e.status, "message": e.message}}),
        status=200,
        content_type=application_json
    )


@superset_app.errorhandler(ValidationError)
def handle_exception(e):
    logger.warning(e.messages)
    msg = json.dumps(e.messages)
    return Response(
        json.dumps({"meta": {"code": 400, "message": msg}}),
        status=200,
        content_type=application_json
    )


@superset_app.errorhandler(404)
def error_date(error):
    logger.warning("Not Found")
    return Response(
        json.dumps({"meta": {"code": 404, "message": "Not Found"}}),
        status=404,
        content_type=application_json
    )


@superset_app.errorhandler(405)
def error_date(error):
    logger.warning("Not Allow")
    return Response(
        json.dumps({"meta": {"code": 405, "message": "Not Allow"}}),
        status=200,
        content_type=application_json
    )
...
def error_date(error):
    logger.warning("SERVE ERROR")
    return Response(
        json.dumps({"meta": {"code": 500, "message": "SERVE ERROR"}}),
        status=200,
        content_type=application_json
    )
