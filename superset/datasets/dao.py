...
class DatasetDAO(BaseDAO):  # pylint: disable=too-many-public-methods
    ...
    def validate_uniqueness(
        ...
        dataset_query = db.session.query(SqlaTable).filter(
            SqlaTable.custom_name == custom_name,
            SqlaTable.table_group_id == table_group_id
        )
    ...
    def find_dataset_column_by_name(
        cls, dataset_id: int, column_name: str
    ) -> Optional[TableColumn]:
        # We want to apply base dataset filters
        dataset = DatasetDAO.find_by_id(dataset_id)
        if not dataset:
    ...
    def exist_column(
        ...
        exist_column = (
            db.session.query(TableColumn)
            .filter(TableColumn.table_id == dataset_id, TableColumn.ref_column != None)
            .one_or_none())
        if exist_column:
            exist_column_dict = {
