...
def import_dataset(
    ...
    if dataset_group_id == 0:
        ...
        group = DatasetGroupDAO.import_group(group_path)
        dataset_group_id = group.id

    config["table_group_id"] = dataset_group_id
    config["custom_name"] = config.get("custom_name", config["table_name"])  # 兼容旧版本
    try:
