...
from superset.const import ENTITIES_PREFIX_QUERIES
...
class ImportSavedQueriesCommand(ImportModelsCommand):
    ...
    prefix = ENTITIES_PREFIX_QUERIES
    schemas: Dict[str, Schema] = {
        "databases/": ImportV1DatabaseSchema(),
        ENTITIES_PREFIX_QUERIES: ImportV1SavedQuerySchema(),
    }
    ...
    def _import(
        ...
        for file_name, config in configs.items():
            if file_name.startswith(ENTITIES_PREFIX_QUERIES):
        ...
        for file_name, config in configs.items():
            if (
                file_name.startswith(ENTITIES_PREFIX_QUERIES)
