...
class ExportChartsCommand(ExportModelsCommand):
    ...
    def _export(model: Slice, export_related: bool = True) -> Iterator[Tuple[str, str]]:
        chart_slug = f"chart_{model.slice_name}_{model.id}"
        ...
        payload["group_path"] = ChartGroupDAO.export_group(model.slice_group_id)
        file_content = yaml.safe_dump(payload, sort_keys=False)

        event_logger.log_with_context(
            action=LogsMessages.LM_EXPORT_CHART,
            log_to_statsd=False,
            title=payload["slice_name"]
        )
